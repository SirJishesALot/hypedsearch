...
datasets, NOD2_data = testing_utils.define_data()

correct_sequences = testing_utils.generate_truth_set(NOD2_data)
...
for i, spectrum in enumerate(spectra):
    ...
    [all_hits.append(x) for x in initial_hits]
...
with open('metadata.txt', 'a') as m:
    m.write('total number of hits: ' + str(total_hits) + ' out of ' + str(tot_ideal_spec_length) + ' potential hits ' + '(' + str(round((total_hits / tot_ideal_spec_length) * 100)) + '%)' + '\n')
    m.write(str(round((total_hits/tot_measured_spec_length) * 100)) + '% of m/z values in input spectrum correlated to a correct hit \n')
